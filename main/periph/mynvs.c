 ...
 * key 键
 */
esp_err_t nvs_save_ac_handle(struct AC_Control* ac,const char *key,size_t ac_size)
{
    esp_err_t err;
    nvs_handle handle;
    size_t size=0;
    ...
    err = nvs_open(AC_HANDLE_STORAGE_NAMESPACE,NVS_READWRITE,&handle);
    if (err != ESP_OK) 
    {
    ...
    //item集合写入 条目
    err = nvs_set_blob(handle, key, ac, ac_size);
    if(err != ESP_OK)
    {
        ESP_LOGI(TAG, "set blob fial");
        return err;
    }
    ESP_LOGI(TAG, "save in nvs success,key = %s,item_size = %d",key,ac_size);
...
struct AC_Control* nvs_get_ac_handle(size_t* len,const char * key)
{
    ...
    ac_handle = (struct AC_Control*) malloc(sizeof(struct AC_Control));
    if(ac_handle == NULL)
    {
        return NULL;
    }
    //读取item
    err = nvs_get_blob(handle, key, ac_handle, len);
    if(err !=ESP_OK)
    {
        return NULL;
    }
